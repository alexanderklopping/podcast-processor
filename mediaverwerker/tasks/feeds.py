...
def _feeds_repo_url():
    ...
    return f"https://x-access-token:{GITHUB_TOKEN}@github.com/{FEEDS_REPO}.git"
...
def push_feeds_to_github():
    ...
    try:
        ...
        if IS_CLOUD and GITHUB_TOKEN:
            git_run = ["git", "config"]
            subprocess.run(
                git_run + ["user.email", "mediaverwerker@github.com"],
                cwd=FEEDS_DIR,
                check=True,
                capture_output=True,
            )
            subprocess.run(
                git_run + ["user.name", "Mediaverwerker"],
                cwd=FEEDS_DIR,
                check=True,
                capture_output=True,
            )
